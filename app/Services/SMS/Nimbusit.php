...
class Nimbusit
{
    ...
    public static function send($mobile, $message, $dltid){
...
//echo "http://nimbusit.biz/api/SmsApi/SendSingleApi?UserID=".self::$USERID."&Password=".self::$PASSWORD."&SenderID=".self::$SENDERID."&Phno=$mobile&Msg=$message";die;
        curl_setopt_array($curl, array(
            CURLOPT_URL => "http://nimbusit.biz/api/SmsApi/SendSingleApi?UserID=".self::$USERID."&Password=".self::$PASSWORD."&SenderID=".self::$SENDERID."&Phno=$mobile&Msg=".urlencode($message)."&EntityID=".self::$ENTITY_ID."&TemplateID=".$dltid,
